...
/**
 * 定时调度任务 - ETL抽取配置（全量备份（时间拉链））
 */
public class FullBakBatchService extends QuartzJobBean {
	private static final Logger LOGGER = LoggerFactory.getLogger(FullBakBatchService.class);
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		...
		LOGGER.info("数据平台全量备份抽取开始.....");
		JobKey jobKey = context.getJobDetail().getKey();
		String transName = jobKey.getName();
		LOGGER.info("转换名称为【{}】", transName);
		if (StringUtils.isNotEmpty(jobKey.getGroup()) && TransConstants.TABLE_TRANS_GROUP_NAME.equals(jobKey.getGroup())) {
			...
			String fullTransType = TransConstants.FULL_TRANS_TYPE_4;
			...
			for (TransInfo etlIfTransInfo : transList) {
				...
				if (etlTransStatusLogs != null && etlTransStatusLogs.size() > 0) {
					...
					try {
						...
						simpleTrans.allTableTrans(etlIfTransInfo.getId(), etlIfTransInfo.getTransName(), etlIfTransInfo.getSrcDbCode(),
								etlIfTransInfo.getTargetDbCode(), etlIfTransInfo.getSrcTbCode(), etlIfTransInfo.getTargetTbCode(),
								etlIfTransInfo.getSrcDescript(), etlIfTransInfo.getTargetDescript(), etlIfTransInfo.getWhereStr());
						// 插入结束日志
	...
	private List<TransInfo> getNeedToSyncTransInfoList(String jobName){
		...
		query.setTransType(TransConstants.TRANS_TYPE_4);
