 ...
 * 自定义结构抽取 - 自定义清空全量抽取
 */
public class TruncateAfterFullTransTableStructureBatchService extends QuartzJobBean {
    private static final Logger LOGGER = LoggerFactory.getLogger(TruncateAfterFullTransTableStructureBatchService.class);
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        LOGGER.info("定时调度任务 - 自定义结构抽取 - 自定义清空全量抽取开始.....");
        ...
        String fullTransType = TransConstants.FULL_TRANS_TYPE_6;
        ...
        for (TransTableStructureInfo tableStructureInfo : transList) {
            ...
            if (etlTransStatusLogs != null && etlTransStatusLogs.size() > 0) {
                ...
                try {
                    LOGGER.debug("======================开始执行【{}】清空全量抽取", tableStructureInfo.getTransName());
                    LOGGER.info("全量抽取[" + tableStructureInfo.getSrcTbCode() + "]表开始start...");
                    Long beginTime = System.currentTimeMillis();
                    simpleTrans.truncateAfterFullTrans(tableStructureInfo.getId(), tableStructureInfo.getTransName(), tableStructureInfo.getPmDataSourceSrcInfo().getCode(),
                            tableStructureInfo.getPmDataSourceTargetInfo().getCode(), tableStructureInfo.getSrcTbCode(), tableStructureInfo.getTargetTbCode(),
                            tableStructureInfo.getPmDataSourceSrcInfo().getName() + "源,表输入(" + tableStructureInfo.getId() + ")",
                            tableStructureInfo.getPmDataSourceTargetInfo().getName() + "目标数据源，表输出(" + tableStructureInfo.getId() + ")", "", tableStructureInfo.getSqlstr());
                    /*simpleTrans.truncateAfterFullTrans(tableStructureInfo.getId(), tableStructureInfo.getTransName(), tableStructureInfo.getPmDataSourceSrcInfo().getCode(),
                            tableStructureInfo.getPmDataSourceTargetInfo().getCode(), tableStructureInfo.getSrcTbCode(), tableStructureInfo.getTargetTbCode(),
                            tableStructureInfo.getSrcDescript(), tableStructureInfo.getTargetDescript(), null, null);*/
                    LOGGER.info("======================转换：{}，执行完毕，共耗时{}ms", tableStructureInfo.getTransName(), (System.currentTimeMillis() - beginTime));
                    // 插入结束日志
