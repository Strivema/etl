...
/** 
 * <pre>
 * <b>类描述:</b>PmErrorTableStructureLog表对应的Action类。
 * <b>作者:</b>李玉强
 * <b>创建日期:</b>2017-11-13 11:03:59
 * </pre>
 */
public class PmErrorTableStructureLogAction extends BaseStruts2Action {
    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = LoggerFactory.getLogger(PmErrorTableStructureLogAction.class);

    /* 定义业务实体对象，用于新增/修改/查看 */
    protected PmErrorTableStructureLog entity;

    /* 定义查询类业务实体对象，用于查询条件对象 */
    protected PmErrorTableStructureLogQuery queryEntity;

    /* 定义服务层操作类 */
    protected PmErrorTableStructureLogService entityService;
	 ...
	 */
	public void init(){
		entity = new PmErrorTableStructureLog();
		queryEntity = new PmErrorTableStructureLogQuery();
	}
    public PmErrorTableStructureLogAction() {
    }
    ...
    public String create() throws Exception {
        if ("toCreate".equals(operate)) {// 转向新增页面
			...
			// 设置Id
			String id = keyGeneratorContainer.getNextKey("pmErrorTableStructureLogId");
            entity.setId(id);
    ...
    public String delete() throws Exception {
		...
		if (StringUtils.isNotEmpty(selectedIds)) {
			...
			for (int i = 0; i < idArray.length; i++) {
                PmErrorTableStructureLogPK pk = new PmErrorTableStructureLogPK();
				pk.setId(idArray[i]);
        ...
        entityService.deleteByPK((PmErrorTableStructureLogPK)entity);
        return PROMPT;
    }
    ...
    public String view() throws Exception {
		...
		PmErrorTableStructureLog pmErrorTableStructureLog =  entityService.selectByPK(entity);
        if (entity == null) {
           throw new Exception("对象不存在,查询条件:" + entity.pkString());
        } else {
			entity = pmErrorTableStructureLog;
		}
    ...
    public String list() throws Exception {
        if ("query".equals(operate)) {// 执行数据库查询操作
            ...
            queryEntity.setSortColumns("err_create_time desc");
    ...
    public String handle() {
        if (StringUtils.isNotEmpty(selectedIds)) {
            ...
            for (int i = 0; i < id.length; i++) {
                ...
                logQuery.setId(id[i]);
                logQuery.setErrStatus("1");
                logQuery.setRemark(entity.getRemark());
                LOGGER.debug("修改数据:{}", logQuery.pkString());
                entityService.updateByPK(logQuery);
            }
        }
        return PROMPT;
    }
	
	public PmErrorTableStructureLog getEntity() {
        return entity;
    }

    public void setEntity(PmErrorTableStructureLog entity) {
        this.entity = entity;
    }

    public PmErrorTableStructureLogQuery getQueryEntity() {
        return queryEntity;
    }

    public void setQueryEntity(PmErrorTableStructureLogQuery queryEntity) {
        this.queryEntity = queryEntity;
    }

    public void setEntityService(PmErrorTableStructureLogService entityService) {
